...
- `awf.operator-workbench.accessibility-small-screen.v1`: CLI/static accessibility and small-screen rationale plus
  future UI gate.
- `awf.operator-workbench.scheduled-agent-usage.v1`: scheduled-agent entrypoints and artifact contract without a fragile
  UI dependency.
...
- `accessibility_small_screen`: CLI/static accessibility and small-screen rationale, deferred UI-specific checks, future
  UI gate, and self-hosted boundary.
...
```json
{
  ...
  "accessibility_small_screen": {
    ...
    "summary": "No local UI is built for Goal 006, so UI-specific accessibility and small-screen verification are not applicable for T015.",
    "accessibility_model": [
  ...
  "scheduled_agent_usage": {
    ...
    "role_entrypoints": [
      {"role": "pm-review", "first_command": "uv run awf operator-status --json"},
      {"role": "orchestrator", "first_command": "uv run awf ready-work --json"},
      {"role": "worker", "first_command": "uv run awf handoff-summary --audience scheduled --json"},
      {"role": "integrator", "first_command": "uv run awf operator-status --json"},
      {"role": "health", "first_command": "uv run awf health-status --deep --json"}
    ],
    "artifact_contract": {
...
## Accessibility And Small-Screen Artifact

The accessibility and small-screen artifact is generated by `uv run awf accessibility-small-screen`. It records why
Goal 006 does not add UI-specific accessibility or small-screen checks while the selected workbench interface remains
CLI/static.

The command is credential-free. It does not call hosted Logfire, hosted Langfuse, GitHub, cloud credentials, external
project tokens, browser automation, or any UI runtime.

Required top-level fields:

- `schema`: literal `awf.operator-workbench.accessibility-small-screen.v1`.
- `generated_at`: ISO-8601 timestamp.
- `generated_by`: command that generated the artifact.
- `scope`: goal, spec, task, and ticket identifiers.
- `selected_interface`: selected interface, currently `cli-static`.
- `ui_built`: false while Goal 006 has no local UI.
- `ui_checks_required_now`: false while no UI behavior exists to verify.
- `summary`: concise rationale.
- `rationale`: evidence-based reasons for deferring UI-only checks.
- `accessibility_model`: text-surface accessibility expectations.
- `small_screen_model`: CLI/static small-screen review expectations.
- `future_ui_gate`: checks required if a future reviewed decision selects a local UI.
- `source_artifacts`: docs and generated artifacts that support the rationale.
- `self_hosted`: credential-free and external-service-required flags.
- `path`: repo-relative artifact path when generated with `--write`.
...
```json
{
  "schema": "awf.operator-workbench.accessibility-small-screen.v1",
  "generated_at": "2026-06-04T00:00:00Z",
  "generated_by": "uv run awf accessibility-small-screen --write --json",
  "scope": {
    "goal": "006-operator-workbench-review-ux",
    "spec_id": "007-operator-workbench-review-ux",
    "task_id": "T015",
  ...
  "path": ".agent-runs/reports/workbench/accessibility/accessibility-small-screen-20260604T000000Z.json"
}
```

## Scheduled-Agent Usage Artifact

The scheduled-agent usage artifact is generated by `uv run awf scheduled-agent-workbench`. It records how scheduled
agents use CLI/static workbench artifacts without a fragile UI dependency.
...
- `schema`: literal `awf.operator-workbench.scheduled-agent-usage.v1`.
- `generated_at`: ISO-8601 timestamp.
- `generated_by`: command that generated the artifact.
- `scope`: goal, spec, task, and ticket identifiers.
- `selected_interface`: selected interface, currently `cli-static`.
- `fragile_ui_dependency`: false while scheduled agents use repo-local commands and artifacts.
- `summary`: concise scheduled-agent usage summary.
- `first_commands`: copy-ready command list for scheduled-agent startup.
- `role_entrypoints`: role-specific first commands, source fields, and allowed actions.
- `artifact_contract`: source-of-truth paths and flags proving no UI session or chat context is required.
- `resilience_rules`: operating rules for scheduled agents.
- `ui_dependency_boundary`: local UI dependency and fallback behavior.
- `source_artifacts`: docs and generated artifacts that support the contract.
- `self_hosted`: credential-free and external-service-required flags.
- `path`: repo-relative artifact path when generated with `--write`.
...
```json
{
  "schema": "awf.operator-workbench.scheduled-agent-usage.v1",
  "generated_at": "2026-06-04T00:00:00Z",
  "generated_by": "uv run awf scheduled-agent-workbench --write --json",
  "scope": {
    "goal": "006-operator-workbench-review-ux",
    "spec_id": "007-operator-workbench-review-ux",
    "task_id": "T016",
  ...
  "selected_interface": "cli-static",
  "fragile_ui_dependency": false,
  "summary": "Scheduled agents use repo-local CLI/static workbench artifacts and exact commands, not a local UI session.",
  "first_commands": [
    "uv run awf scheduled-agent-workbench --json",
    "uv run awf handoff-summary --audience scheduled --json",
    "uv run awf operator-status --json"
  ...
  "path": ".agent-runs/reports/workbench/scheduled-agents/scheduled-agent-workbench-20260604T000000Z.json"
}
```
