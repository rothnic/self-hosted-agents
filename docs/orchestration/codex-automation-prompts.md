...
Use repo artifacts as source of truth. Run `uv run awf automation-loop --role pm-review --write --json`.
...
Run `uv run awf automation-loop --role orchestrator --write --json`.
...
Run `uv run awf automation-loop --role worker --worker-id <stable-worker-id> --write --json`.
...
branch. Run `uv run awf automation-loop --role integrator --write --json` and
...
Run `uv run awf automation-loop --role health --write --json`.
