...
Use repo artifacts as source of truth. Set `UV_CACHE_DIR` to a writable path outside the repo, then run
`uv run awf automation-loop --role pm-review --write --json`.
...
Set `UV_CACHE_DIR` to a writable path outside the repo, then run
`uv run awf automation-loop --role orchestrator --write --json`.
...
Set `UV_CACHE_DIR` to a writable path outside the repo, then run
`uv run awf automation-loop --role worker --worker-id <stable-worker-id> --write --json`.
...
branch. Set `UV_CACHE_DIR` to a writable path outside the repo, then run
`uv run awf automation-loop --role integrator --write --json` and
`uv run awf verify --profile increment --write --json`. Prepare the human review PR to `main` when the increment is
